![Pasted image 20260221205320](../assets/images/Pasted%20image%2020260221205320.png)
竖着的三列 就是我们要做的三个步骤
```js
...
        while(nums[j]<=nums[i])
////i右边的一定是倒序的 所以我们从后往左遍历第一个跳出循环满足nums[j]>nums[i]  即从右向左遍历 查看最小 比nums[i] 大的数
//就是最下大于的元素索引 这里是<=
            j--
...
    while(left<right){//包括i小于0 即整体反转 还是找到i 我们反转一部分的情况
	//第三步 我们反转 索引 从i+1到最后的 右半部分 将降序（左大于右）改为升序（左小于右）
